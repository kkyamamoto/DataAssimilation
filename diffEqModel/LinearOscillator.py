...
class LinearOscillator:
    def __init__(self, kfInitialMean=10, noiseCov=50, finalTime=100, timeIncrement=1e-3, omega=1, xi=1e-3,
                 modelInitial=[1, 2]):
    ...
    def computeMSE(self):
        self.mse=np.full(self.totalTimeSteps+1,np.nan)
        for n in range(self.totalTimeSteps+1):
            self.mse[n]=((self.trueState[n][0]-self.kfReconstructionMean[n][0])**2+(self.trueState[n][1]-self.kfReconstructionMean[n][1])**2)/2
